...
int pf_movement_bit_count(void) { return pf::movement_bit_count(); }
unsigned pf_movement_bit_value(int i) { return unsigned(pf::movement_bit_value(i)); }
const char* pf_movement_bit_name(int i) { return pf::movement_bit_name(i); }
...
int pf_map_validate(const pf_map* map, pf_issue* out, int capacity) {
  ...
  for (size_t i = 0; i < m.units().size(); i++) {
    ...
    for (size_t j = i + 1; j < m.units().size(); j++) {
      ...
      if (apart) continue;
      if (pf::units_may_share_tiles(a.type, b.type)) continue;
  ...
  if (o->overlay != PF_OVERLAY_NONE) {
    ...
    for (int row = 0; row < o->rows; row++) {
      for (int col = 0; col < o->cols; col++) {
        ...
        // Over the terrain rather than instead of it: the layer is what is
        // being edited, but the coastline under it is how anybody knows where
        // to paint. High enough that a value still reads as its palette colour
        // — at 55% the same green came out one shade on grass and another on
        // snow, and no two cells could be told apart on sight.
        //
        // Fixed point, hoisted out of the pixel loop: the tint and the weight
        // are the same for all 1,024 pixels of the tile, and this runs over
        // every pixel of the view on every repaint.
        const uint32_t w = 200u;                  // 0.78 of 256
        const uint32_t v = 256u - w;
